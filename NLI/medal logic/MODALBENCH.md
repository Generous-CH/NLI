...
table1
k 无任何约束，T自反（自己可以到达自己），S4自反+传递（a到b，b到c，则a到c），S5自反+传递+对称(a到b，则b到a),D序列性(每个世界至少有一个可达世界）。
...
三种模型，三种提示策略:Zero shot ,Chain of thought(在问题后加上“Think step by step.”（但不指定具体步骤),World-Enumeration CoT提供一个**结构化的分步模板**（论文附录 B），明确指示模型：  列出所有可达世界,对每个子公式在每个可达世界求值,用 ∀（全称）或 ∃（存在）聚合,递归处理嵌套模态,最后输出“True”或“False”,用于测试**外部化 Kripke 评价算法**是否能作为“脚手架”（scaffold）帮助模型。这本质上是在模拟一个符号求解器的执行流程。
- chain-of-thought 是“自由发挥”的逐步推理，模型自己决定怎么组织思路。
- World-Enumeration 是“强制模板”，模型必须按 Kripke 语义的机械步骤来走，没有自由发挥空间。
**题目总数**：3,000 道（1,500 个独特 Kripke 实例 × 2 个轨道：形式化 + 自然语言）。**模型数**：3 个。**提示策略数**：3 种。**总推理次数**：3 个模型×3 种策略×3,000 道=27,000 次推理**随机种子固定**：所有生成都是确定性的（见 Reproducibility Statement）。**标签平衡**：如前所述，每个 cell 严格 50/50，排除了“偏向多数类”的捷径。 **统计检验**：所有主要结果都附带 **p 值**（ANOVA 或 t-test），证明差异不是随机波动造成的。


论文发现：**两个模型在自然语言轨道上的准确率显著高于形式化符号轨道**，挑战了“符号化表示对逻辑推理更友好”的传统假设。**提示策略不是“越复杂越好”，而是“越匹配模型能力越好”。**
论文发现：**所有模型在系统 K（无任何可达关系约束）中，都系统性过度应用了“公理 5（Euclidean 性质）：⋄P→□⋄P⋄P→□⋄P”——而这条公理只在 S5（等价关系）中才有效。**
...
我能从这篇论文学到什么：
之前我并没有考虑自反，传递，对称，要注意分类自己能不能到达自己
（1）kripke Model的程序表示
